  ...
  return (
    <div className={cn("min-h-screen bg-background pb-20 text-foreground md:pb-8", focusMode && "[&_header]:opacity-40 [&_header]:hover:opacity-100")}>
        ...
        <div className="mx-auto flex h-14 w-full max-w-5xl items-center justify-between gap-3 px-4 sm:px-6">
          ...
          <nav aria-label="Navigasi utama" className="hidden md:block">
            <ul className="flex items-center gap-0.5">
              {navItems.map((item) => <li key={item.to}><Link to={item.to} aria-current={isActive(item.to) ? "page" : undefined} className={cn("rounded-md px-2.5 py-1.5 text-[13px] font-medium transition-colors", isActive(item.to) ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground")}>{item.label}</Link></li>)}
            </ul>
          </nav>
      ...
      <main className="mx-auto w-full max-w-5xl px-4 py-5 sm:px-6 sm:py-7">
        ...
        <div className="mb-5">
          {backTo && <Link to={backTo} className="mb-2 inline-flex items-center gap-1.5 text-[13px] text-muted-foreground hover:text-foreground">← {backLabel}</Link>}
      ...
      <nav aria-label="Navigasi bawah" className="fixed inset-x-0 bottom-0 z-30 border-t border-border/60 bg-background/96 backdrop-blur-xl md:hidden">
        <ul className="mx-auto flex max-w-lg items-stretch">
          {navItems.map((item) => { const active = isActive(item.to); const Icon = item.icon; return <li key={item.to} className="flex-1"><Link to={item.to} aria-current={active ? "page" : undefined} className={cn("flex min-h-14 flex-col items-center justify-center gap-0.5 text-[10px] font-medium", active ? "text-primary" : "text-muted-foreground")}><Icon className="size-[18px]" />{item.label}</Link></li>; })}
        </ul>
      </nav>
    </div>
  );
