...
type Props = {
  ...
  examples?: Example[]; relatedWords?: RelatedWord[];
  ...
  return <div className="mx-auto max-w-2xl">
    ...
    <Card className="overflow-hidden border-border/80 shadow-xl">
      <CardHeader className="space-y-4 bg-gradient-to-br from-primary/10 via-background to-secondary/20 px-6 py-8 text-center sm:px-8">
        <div className="flex items-center justify-center gap-2"><Badge variant="secondary">{level}</Badge><Badge variant="outline">Kanji</Badge></div>
        ...
        <div className="flex flex-wrap justify-center gap-2"><Button type="button" variant="outline" size="sm" onClick={() => speak(title)}>🔊 Dengarkan kanji</Button><Button type="button" variant="ghost" size="sm" onClick={() => setShowDetails(v => !v)}>{showDetails ? "Sembunyikan rincian" : "Buka rincian lengkap"}</Button></div>
      </CardHeader>
      <CardContent className="space-y-6 p-5 sm:p-7">
        {showDetails && <div className="space-y-6 animate-in fade-in-0 slide-in-from-top-2 duration-300">
          {(onyomi?.length || kunyomi?.length) ? <section className="rounded-2xl border bg-muted/30 p-5"><p className={label}>CARA MEMBACA</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><div className="rounded-xl bg-background p-4"><p className="font-semibold">音読み — Onyomi</p><p lang="ja" className="mt-2 font-jp text-lg">{onyomi?.length ? onyomi.join(" ・ ") : "Tidak tersedia"}</p><p className="mt-2 text-sm text-muted-foreground">Umumnya digunakan ketika kanji menjadi bagian dari kata gabungan.</p></div><div className="rounded-xl bg-background p-4"><p className="font-semibold">訓読み — Kunyomi</p><p lang="ja" className="mt-2 font-jp text-lg">{kunyomi?.length ? kunyomi.join(" ・ ") : "Tidak tersedia"}</p><p className="mt-2 text-sm text-muted-foreground">Umumnya digunakan ketika kanji berdiri sebagai kata Jepang asli atau bersama okurigana.</p></div></div></section> : null}
          {structure && <section className="rounded-2xl border bg-muted/30 p-5"><p className={label}>POLA / STRUKTUR</p><p lang="ja" className="mt-3 whitespace-pre-line font-jp text-lg leading-8">{structure}</p></section>}
          ...
          {question && <section className="rounded-2xl border-2 border-primary/20 bg-primary/5 p-5"><p className={label + " text-primary"}>UJI PEMAHAMAN</p><p className="mt-3 font-medium leading-7">{question.prompt}</p><div className="mt-4 grid gap-3">{question.choices.map((choice, i) => <Button key={`${choice}-${i}`} type="button" variant={selected === i ? (i === question.correctIndex ? "secondary" : "destructive") : "outline"} className="h-auto min-h-12 justify-start whitespace-normal py-3 text-left" onClick={() => setSelected(i)}>{String.fromCharCode(65 + i)}. {choice}</Button>)}</div>{selected !== null && <div className="mt-4 rounded-xl border bg-background p-4 text-sm leading-6"><p className="font-semibold">{selected === question.correctIndex ? "✓ Jawaban benar" : "✗ Jawaban belum tepat"}</p>{selected !== question.correctIndex && <p className="mt-1">Jawaban yang benar: {String.fromCharCode(65 + question.correctIndex)}.</p>}<p className="mt-2">Baca kembali penjelasan dan contoh di atas sebelum melanjutkan.</p></div>}</section>}
        </div>}
