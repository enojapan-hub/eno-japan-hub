...
import { ArrowLeft, ArrowRight, Check, ChevronDown, ChevronUp, Volume2 } from "lucide-react";
...
export function StudyFlashcard({index,total,level,title,reading,romaji,meaning,secondary,explanation,examples,relatedWords,onyomi,kunyomi,usageNotes,commonMistakes,question,furiganaEnabled=true,kotobaMode=false,kanjiMode=false,onPrev,onNext,onLearned,learned}:Props){
 ...
 useEffect(()=>{setShowDetails(true);setQuizChoice(null);window.speechSynthesis?.cancel()},[index,title]);
 ...
 return <div className="mx-auto max-w-2xl">
  ...
  <Card className="overflow-hidden border-border/80 shadow-xl">
   <CardHeader className="space-y-4 bg-gradient-to-br from-primary/10 via-background to-secondary/20 px-6 py-8 text-center sm:px-8">
    <div className="flex items-center justify-center gap-2"><Badge variant="secondary">{level}</Badge><Badge variant="outline">{kanjiMode?"KANJI":kotobaMode?"KOTOBA":"Pelajaran"}</Badge></div>
    {kanjiMode?<div lang="ja" className="font-jp text-8xl font-bold leading-none tracking-wide sm:text-9xl">{furiganaEnabled&&reading?<ruby>{title}<rt className="mb-2 text-xl font-normal text-muted-foreground sm:text-2xl">{reading}</rt></ruby>:title}</div>:<div lang="ja" className="font-jp text-5xl font-bold tracking-wide sm:text-6xl">{title}{kotobaMode&&reading&&<span className="ml-3 text-2xl font-normal text-muted-foreground sm:text-3xl">— {reading}</span>}</div>}
    ...
    {!kanjiMode&&secondary&&<p className="text-sm text-muted-foreground">{secondary}</p>}
    <div className="flex flex-wrap justify-center gap-2"><Button type="button" variant="outline" size="sm" onClick={()=>speak(title)}><Volume2 className="mr-2 size-4"/><span>Dengarkan</span></Button><Button type="button" variant="ghost" size="sm" onClick={()=>setShowDetails(v=>!v)}>{showDetails?<ChevronUp className="mr-2 size-4"/>:<ChevronDown className="mr-2 size-4"/>}<span>{showDetails?"Sembunyikan penjelasan":"Tampilkan penjelasan lengkap"}</span></Button></div>
   </CardHeader>
   <CardContent className="space-y-6 p-5 sm:p-7">
    {showDetails&&<div className="space-y-6">
     {kanjiMode?<>
      <section className="rounded-2xl border bg-muted/30 p-5"><p className={sectionTitle}>KUNYOMI</p><p lang="ja" className="mt-3 font-jp text-xl leading-8">{kunyomi?.length?kunyomi.join("・"):"—"}</p></section>
      <section className="rounded-2xl border bg-muted/30 p-5"><p className={sectionTitle}>ONYOMI</p><p lang="ja" className="mt-3 font-jp text-xl leading-8">{onyomi?.length?onyomi.join("・"):"—"}</p></section>
      <section className="rounded-2xl border bg-muted/30 p-5"><p className={sectionTitle}>KOTOBA TERKAIT — kata yang menggunakan kanji ini</p>{relatedWords?.length?<div className="mt-4 grid gap-3 sm:grid-cols-2">{relatedWords.slice(0,10).map((w,i)=><div key={`${w.term}-${i}`} className="rounded-xl border bg-background p-4"><p lang="ja" className="font-jp text-2xl font-semibold">{w.term}</p>{furiganaEnabled&&w.reading&&<p lang="ja" className="mt-1 text-sm text-muted-foreground">{w.reading}</p>}<p className="mt-2 text-sm leading-6"><span className="font-medium">Arti :</span> {w.meaning||"Arti belum tersedia"}</p>{w.example&&<p lang="ja" className="mt-2 border-t pt-2 font-jp text-sm leading-6">{w.example}</p>}<Button type="button" size="icon" variant="ghost" className="mt-1" aria-label={`Dengarkan ${w.term}`} onClick={()=>speak(w.term)}><Volume2 className="size-4"/></Button></div>)}</div>:<p className="mt-3 text-sm text-muted-foreground">Belum ada Kotoba yang menggunakan kanji ini.</p>}</section>
      <section className="rounded-2xl border bg-muted/30 p-5"><p className={sectionTitle}>CONTOH KALIMAT</p>{usableExamples.length?<div className="mt-4 space-y-3">{usableExamples.map((example,i)=><div key={`${example.jp??"contoh"}-${i}`} className="rounded-xl border bg-background p-4"><div className="flex items-start gap-2"><div className="flex-1">{example.jp&&<p lang="ja" className="font-jp text-lg leading-8">{example.jp}</p>}{example.reading&&furiganaEnabled&&<p lang="ja" className="mt-1 text-sm text-muted-foreground">{example.reading}</p>}{example.id&&<p className="mt-2 text-sm leading-6">{example.id}</p>}</div>{example.jp&&<Button type="button" size="icon" variant="ghost" aria-label="Dengarkan contoh" onClick={()=>speak(example.jp!)}><Volume2 className="size-4"/></Button>}</div></div>)}</div>:<p className="mt-3 text-sm text-muted-foreground">Belum ada contoh kalimat.</p>}</section>
      {activeQuestion&&<section className="rounded-2xl border-2 border-primary/20 bg-primary/5 p-5"><p className="text-xs font-bold tracking-wide text-primary">QUIZ KANJI</p><p className="mt-3 font-medium leading-7">{activeQuestion.prompt}</p><div className="mt-4 grid gap-3">{activeQuestion.choices.map((choice,i)=>{const selected=quizChoice===i;const correct=answered&&i===activeQuestion.correctIndex;const wrong=selected&&!correct;return <Button key={`${choice}-${i}`} type="button" variant={correct?"default":wrong?"destructive":selected?"secondary":"outline"} className="h-auto min-h-12 justify-start whitespace-normal py-3 text-left" onClick={()=>!answered&&setQuizChoice(i)} disabled={answered}>{String.fromCharCode(65+i)}. {choice}</Button>})}</div>{answered&&<div className="mt-4 rounded-xl border bg-background p-3 text-sm">{quizChoice===activeQuestion.correctIndex?<span className="font-semibold text-emerald-600">Benar! Jawabanmu tepat.</span>:<span className="font-semibold text-destructive">Belum tepat. Coba perhatikan kembali bacaan dan Kotoba terkait di atas.</span>}<Button type="button" variant="ghost" size="sm" className="mt-2 w-full" onClick={()=>setQuizChoice(null)}>Coba lagi</Button></div>}</section>}
     </>:<>
