 ...
 return <div className="mx-auto max-w-2xl">
  ...
  <Card className="overflow-hidden border-border/80 shadow-xl">
   ...
   <CardContent className="space-y-6 p-5 sm:p-7">
    ...
    <div className="grid grid-cols-2 gap-2 border-t pt-5 sm:grid-cols-3"><Button variant="outline" onClick={onPrev} disabled={index===0}><ArrowLeft className="mr-2 size-4" strokeWidth={2}/>Sebelumnya</Button>{onLearned&&<Button variant={learned?"secondary":"outline"} onClick={onLearned}>{learned?<><Check className="mr-2 size-4" strokeWidth={2}/>Sudah dipelajari</>:"Tandai sudah dipelajari"}</Button>}<Button className="col-span-2 sm:col-span-1" onClick={onNext} disabled={index===total-1}>Berikutnya<ArrowRight className="ml-2 size-4" strokeWidth={2}/></Button></div>
   </CardContent>
  </Card>
 </div>;
}
