...
const sectionLabel = "text-[11px] font-semibold tracking-[0.14em] text-muted-foreground";

function withFurigana(text: string, enabled: boolean) {
  if (!enabled) return text;
  return text;
}

export function StudyFlashcard({ index, total, level, title, reading, meaning, secondary, structure, explanation, examples, question, furiganaEnabled = true, onPrev, onNext, onLearned, learned }: Props) {
  const [showDetails, setShowDetails] = useState(true);
  const [selected, setSelected] = useState<number | null>(null);
  const progress = total ? Math.round(((index + 1) / total) * 100) : 0;
  const usableExamples = useMemo(() => (examples ?? []).filter((x) => x.jp || x.id || x.reading).slice(0, 5), [examples]);
  ...
  return <div className="mx-auto max-w-2xl">
    ...
    <div className="mb-5 h-1.5 overflow-hidden rounded-full bg-muted" aria-label={`Kemajuan ${progress} persen`}><div className="h-full rounded-full bg-primary transition-all duration-300" style={{ width: `${progress}%` }} /></div>
    <Card className="overflow-hidden border-border/80 shadow-xl">
      <CardHeader className="space-y-4 bg-gradient-to-br from-primary/10 via-background to-secondary/20 px-6 py-8 text-center sm:px-8">
        ...
        {reading && <div lang="ja" className="text-base text-muted-foreground">{withFurigana(reading, furiganaEnabled)}</div>}
        ...
        <div className="flex justify-center"><Button type="button" variant="outline" size="sm" onClick={() => speak(`${title}${reading ? ` ${reading}` : ""}`)}>🔊 Dengarkan pengucapan</Button></div>
      </CardHeader>
