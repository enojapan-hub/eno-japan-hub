...
type Props = { index:number; total:number; level:string; title:string; reading?:string|null; meaning:string; secondary?:string|null; structure?:string|null; explanation?:string|null; examples?:Example[]; relatedWords?:RelatedWord[]; onyomi?:string[]; kunyomi?:string[]; question?:{prompt:string;choices:string[];correctIndex:number}|null; furiganaEnabled?:boolean; onPrev:()=>void; onNext:()=>void; onLearned?:()=>void; learned?:boolean };
  ...
  <Card className="overflow-hidden border-border/80 shadow-xl">
   ...
   <CardContent className="space-y-6 p-5 sm:p-7">
    {showDetails&&<div className="space-y-6">
     {(onyomi?.length||kunyomi?.length)&&<section className="rounded-2xl border bg-muted/30 p-5"><p className="text-[11px] font-semibold tracking-[0.14em] text-muted-foreground">CARA MEMBACA</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><div className="rounded-xl bg-background p-4"><p className="font-semibold">音読み（おんよみ）</p><p lang="ja" className="mt-2 font-jp text-lg">{onyomi?.length?onyomi.join(" ・ "):"Belum tersedia"}</p><p className="mt-2 text-sm text-muted-foreground">Umumnya digunakan ketika kanji menjadi bagian dari kata gabungan.</p></div><div className="rounded-xl bg-background p-4"><p className="font-semibold">訓読み（くんよみ）</p><p lang="ja" className="mt-2 font-jp text-lg">{kunyomi?.length?kunyomi.join(" ・ "):"Belum tersedia"}</p><p className="mt-2 text-sm text-muted-foreground">Umumnya digunakan pada kata Jepang asli, termasuk kata yang memakai okurigana.</p></div></div></section>}
     {structure&&<section className="rounded-2xl border bg-muted/30 p-5"><p className="text-[11px] font-semibold tracking-[0.14em] text-muted-foreground">POLA DAN STRUKTUR</p><p lang="ja" className="mt-3 whitespace-pre-line font-jp text-lg leading-8">{structure}</p></section>}
     {explanation&&<section className="rounded-2xl border bg-muted/30 p-5"><p className="text-[11px] font-semibold tracking-[0.14em] text-muted-foreground">PENJELASAN LENGKAP</p><p className="mt-3 whitespace-pre-line leading-7">{explanation}</p></section>}
     {!!relatedWords?.length&&<section className="rounded-2xl border bg-muted/30 p-5"><p className="text-[11px] font-semibold tracking-[0.14em] text-muted-foreground">KOSAKATA TERKAIT</p><div className="mt-4 grid gap-3 sm:grid-cols-2">{relatedWords.slice(0,8).map((w,i)=><div key={`${w.term}-${i}`} className="rounded-xl border bg-background p-4"><div className="flex items-start justify-between gap-2"><div><p lang="ja" className="font-jp text-xl">{w.term}</p>{furiganaEnabled&&w.reading&&<p lang="ja" className="mt-1 text-sm text-muted-foreground">{w.reading}</p>}<p className="mt-2 text-sm">{w.meaning||"Arti belum tersedia"}</p></div><Button type="button" size="icon" variant="ghost" aria-label={`Dengarkan ${w.term}`} onClick={()=>speak(`${w.term} ${w.reading??""}`)}>🔊</Button></div></div>)}</div></section>}
     {!!usableExamples.length&&<section className="rounded-2xl border bg-muted/30 p-5"><p className="text-[11px] font-semibold tracking-[0.14em] text-muted-foreground">CONTOH KALIMAT</p><div className="mt-4 space-y-4">{usableExamples.map((example,i)=><div key={`${example.jp??"contoh"}-${i}`} className="rounded-xl border bg-background p-4"><div className="flex items-start gap-2"><div className="flex-1">{example.jp&&<p lang="ja" className="font-jp text-lg leading-8">{example.jp}</p>}{example.reading&&furiganaEnabled&&<p lang="ja" className="mt-1 text-sm text-muted-foreground">{example.reading}</p>}{example.id&&<p className="mt-2 text-sm leading-6">{example.id}</p>}</div>{example.jp&&<Button type="button" size="icon" variant="ghost" aria-label="Dengarkan contoh kalimat" onClick={()=>speak(example.jp!)}>🔊</Button>}</div></div>)}</div></section>}
     {question&&<section className="rounded-2xl border-2 border-primary/20 bg-primary/5 p-5"><p className="text-[11px] font-semibold tracking-[0.14em] text-primary">LATIHAN PEMAHAMAN</p><p className="mt-3 font-medium leading-7">{question.prompt}</p><div className="mt-4 grid gap-3">{question.choices.map((choice,i)=><Button key={`${choice}-${i}`} type="button" variant={selected===i?(i===question.correctIndex?"secondary":"destructive"):"outline"} className="h-auto min-h-12 justify-start whitespace-normal py-3 text-left" onClick={()=>setSelected(i)}>{String.fromCharCode(65+i)}. {choice}</Button>)}</div>{selected!==null&&<div className="mt-4 rounded-xl border bg-background p-4 text-sm leading-6"><p className="font-semibold">{selected===question.correctIndex?"✓ Jawaban benar":"✗ Jawaban belum tepat"}</p>{selected!==question.correctIndex&&<p className="mt-1">Jawaban yang benar: {String.fromCharCode(65+question.correctIndex)}.</p>}<p className="mt-2">Perhatikan kembali penjelasan dan contoh penggunaan sebelum melanjutkan.</p></div>}</section>}
    </div>}
