...
export async function fetchKanjiList(level: Level) {
  return must(
    await supabase
      .from("kanji")
      .select("id, character, level, onyomi, kunyomi, meaning_id, stroke_count, sort_order")
      .eq("level", level)
...
export async function fetchKanjiDetail(id: string) {
  ...
  const relations = must(
    await supabase
      ...
      .select("id, note_id, sort_order, related:related_kanji_id (id, character, meaning_id, level)")
      .eq("kanji_id", id)
      .order("sort_order"),
  );
...
export async function fetchGrammarList(level: Level) {
  return must(
    await supabase
      .from("grammar_points")
      .select("id, pattern, meaning_id, structure, explanation_id, examples, level, sort_order")
      .eq("level", level)
      .eq("is_published", true)
      .order("sort_order"),
  );
...
export async function fetchPassages() {
  return must(
    await supabase
      ...
      .select("id, title, level, body_jp, translation_id, estimated_minutes, sort_order")
      .eq("is_published", true)
      .order("sort_order"),
  );
}
...
export async function fetchPassageDetail(id: string) {
  const passages = must(
    await supabase.from("reading_passages").select("*").eq("id", id).limit(1),
  );
...
export async function fetchListeningList() {
  return must(
    await supabase
      ...
      .select("id, title, level, duration_seconds, sort_order")
      .eq("is_published", true)
      .order("sort_order"),
  );
}
...
export async function fetchListeningDetail(id: string) {
  const items = must(await supabase.from("listening_items").select("*").eq("id", id).limit(1));
  const questions = must(
    await supabase
      .from("questions")
      .select("id, prompt, choices, correct_index, explanation_id")
...
export async function fetchQuizzes() {
  return must(
    await supabase
      .from("quizzes")
      .select("id, slug, title, description, level, skill, question_count, time_limit_seconds")
      .eq("is_published", true)
      .order("level")
      .order("sort_order"),
  );
...
export async function fetchQuizBySlug(slug: string) {
  ...
  const rows = must(
    await supabase
      .from("quiz_questions")
      .select("sort_order, question:question_id (id, prompt, prompt_note, choices, correct_index, explanation_id, is_published)")
      .eq("quiz_id", quiz.id)
      .order("sort_order"),
  ) as Array<{ question: Record<string, unknown> | null }>;
  const questions = toRunnerQuestions(
    rows.map((r) => r.question).filter((q): q is Record<string, unknown> => !!q && q['is_published'] === true),
  );
...
export async function fetchSimulationQuestions(level: Level, group: SimSkillGroup) {
  const skills =
    group === "vocabulary_grammar"
      ? (["vocabulary", "grammar", "kanji"] as const)
      : group === "reading"
        ? (["reading"] as const)
        : (["listening"] as const);
  ...
  return toRunnerQuestions(rows);
}

/** Simpan hasil kuis/simulasi. Gagal menyimpan tidak boleh merusak pengalaman belajar. */
...
export async function markItemLearned(input: {
  ...
  await supabase.from("user_item_progress").insert({
    user_id: userId,
...
export async function fetchMyProgress() {
  ...
  return {
    ...
    weak: (answers.data ?? []) as Array<{ question: { skill: string; prompt: string } | null }>,
  };
}
...
export async function fetchRewards() {
  ...
  const [profile, stats, referrals, grants] = await Promise.all([
    ...
    supabase.from("user_stats").select("reward_points, total_xp, current_streak").eq("user_id", userId).maybeSingle(),
    supabase
      .from("referrals")
