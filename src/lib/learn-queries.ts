...
export async function fetchKanjiStudy(id: string) {
  ...
  for (const v of vocabRows as Array<Record<string, unknown>>) {
    relatedWords.push({ term: String(v["term"] ?? ""), reading: v["reading"] ? String(v["reading"]) : null, meaning: v["meaning_id"] ? String(v["meaning_id"]) : null });
...
export async function fetchListeningList() { return must(await supabase.from("listening_items").select("id, title, level, duration_seconds, sort_order, audio_url, source").eq("is_published", true).order("sort_order")); }
export async function fetchListeningDetail(id: string) { const items = must(await supabase.from("listening_items").select("*").eq("id", id).limit(1)); const questions = must(await supabase.from("questions").select("id, prompt, choices, correct_index, explanation_id").eq("listening_id", id).eq("is_published", true)); return { item: items[0] ?? null, questions }; }
