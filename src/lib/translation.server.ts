...
const SOURCE_DEFINITIONS = [
  ...
  { type: 'grammar', table: 'grammar_points', fields: [{ source: 'meaning_en', target: 'meaning_id' }] },
  { type: 'reading', table: 'reading_passages', fields: [{ source: 'title', target: 'title' }] },
  { type: 'vocab', table: 'vocabulary', fields: [{ source: 'meaning_en', target: 'meaning_id' }] },
  { type: 'vocab_items', table: 'vocabulary_items', fields: [{ source: 'meaning_en', target: 'meaning_id' }] },
  { type: 'words', table: 'words', fields: [{ source: 'meaning_en', target: 'meaning_id' }] },
] as const
...
function looksIndonesian(value: string | null | undefined) {
  ...
  const markers = [' yang ', ' dan ', ' untuk ', ' dengan ', ' dari ', ' dalam ', ' adalah ', ' berarti ', ' digunakan ']
...
async function discoverWork(limit: number) {
  const jobs: Array<{ type: string; id: string; sourceField: string; targetField: string; sourceText: string; context: string }> = []

  for (const definition of SOURCE_DEFINITIONS) {
    if (jobs.length >= limit) break
    const table = definition.table
    const fields = definition.fields[0]
    const { data, error } = await (supabaseAdmin as any)
      .from(table)
    ...
    for (const row of data) {
      ...
      jobs.push({ type: definition.type, id: row.id, sourceField: fields.source, targetField: fields.target, sourceText, context: definition.type })
    }
  }
  return jobs
}
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      const { error: updateError } = await (supabaseAdmin as any)
        .from(SOURCE_DEFINITIONS.find((d) => d.type === job.type)?.table ?? job.type)
        .update({ [job.targetField]: translated.translation, updated_at: new Date().toISOString() })
        .eq('id', job.id)
