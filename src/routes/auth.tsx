...
function AuthPage() {
  ...
  useEffect(() => { let active = true; supabase.auth.getSession().then(async ({ data }) => { if (!active) return; if (data.session) await continueAfterAuth(navigate); else setChecking(false); }); return () => { active = false; }; }, [navigate]);
  ...
  async function submitEmailAuth(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (mode === "signup") {
        ...
        if (data.session) await continueAfterAuth(navigate); else { toast.success("Pendaftaran berhasil. Periksa email untuk verifikasi akun."); setMode("login"); }
      } else {
        const { error: signInError } = await supabase.auth.signInWithPassword({ email: normalizedEmail, password });
        if (signInError) throw signInError; await continueAfterAuth(navigate);
      }
    } catch (caught) { const message = caught instanceof Error ? caught.message : "Autentikasi gagal."; setError(message); toast.error(message); }
    finally { setLoading(false); }
  }

  async function signInWithGoogle() {
    setError(null); setLoading(true);
    try { const { error: oauthError } = await supabase.auth.signInWithOAuth({ provider: "google", options: { redirectTo: getAuthRedirectUrl() } }); if (oauthError) throw oauthError; }
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#f7f7f4] px-0 py-0 sm:px-5 sm:py-8">
      <section className="relative flex min-h-screen w-full max-w-[380px] flex-col overflow-hidden bg-white shadow-2xl shadow-[#1f6f4a]/20 sm:min-h-[720px] sm:rounded-[36px]">
        {/* Header hanya logo, tanpa teks atau ornamen */}
        <header className="flex h-[220px] shrink-0 items-center justify-center bg-white">
          <Link to="/" aria-label="ENO NIHONGO"><BrandMark size="lg" /></Link>
        </header>
        <div className="flex flex-1 flex-col bg-white px-6 pb-6 pt-7 sm:px-7">
          <div className="mb-5"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#1f6f4a]">{isLogin ? "Selamat datang" : "Buat akun"}</p><h2 className="mt-1.5 text-[1.45rem] font-bold leading-tight tracking-tight text-[#263b31]">{isLogin ? "Masuk ke akunmu" : "Mulai belajar bersama kami"}</h2><p className="mt-1.5 text-xs leading-5 text-[#63756d]">{isLogin ? "Lanjutkan belajar bahasa Jepang dan capai target JLPT-mu." : "Simpan progres belajar dan bangun kebiasaan bahasa Jepangmu."}</p></div>
          <div className="mb-5 grid grid-cols-2 rounded-xl bg-[#eef5f0] p-1"><button type="button" onClick={() => setMode("login")} className={`h-10 rounded-lg text-sm font-semibold ${isLogin ? "bg-white text-[#24704d] shadow-sm" : "text-[#718078]"}`}>Masuk</button><button type="button" onClick={() => setMode("signup")} className={`h-10 rounded-lg text-sm font-semibold ${!isLogin ? "bg-white text-[#24704d] shadow-sm" : "text-[#718078]"}`}>Daftar</button></div>
          <form onSubmit={submitEmailAuth} className="space-y-3.5" noValidate>
            <div className="space-y-1.5"><Label htmlFor="auth-email" className="text-xs font-semibold text-[#30483c]">Email</Label><div className="relative"><Mail aria-hidden className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-[#8aa096]" /><Input id="auth-email" type="email" className="h-11 rounded-xl border-[#dce8e2] bg-white pl-9 text-sm shadow-sm focus-visible:ring-[#1f6f4a]" value={email} onChange={e => setEmail(e.target.value)} autoComplete="email" required /></div></div>
            <div className="space-y-1.5"><div className="flex items-center justify-between gap-3"><Label htmlFor="auth-password" className="text-xs font-semibold text-[#30483c]">Kata sandi</Label>{isLogin && <Link to="/reset-password" className="text-[11px] font-medium text-[#1f6f4a] underline underline-offset-4">Lupa kata sandi?</Link>}</div><div className="relative"><Lock aria-hidden className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-[#8aa096]" /><Input id="auth-password" type="password" className="h-11 rounded-xl border-[#dce8e2] bg-white pl-9 text-sm shadow-sm focus-visible:ring-[#1f6f4a]" value={password} onChange={e => setPassword(e.target.value)} autoComplete={isLogin ? "current-password" : "new-password"} required /></div></div>
          ...
          <Button type="button" variant="outline" className="h-11 w-full rounded-xl border-[#dce8e2] bg-white text-sm font-medium text-[#30483c] shadow-sm hover:bg-[#f1f7f3]" onClick={signInWithGoogle} disabled={loading}><span className="mr-2 grid size-5 place-items-center rounded-full bg-[#e5f3eb] text-[11px] font-bold text-[#1f6f4a]">G</span>Lanjutkan dengan Google</Button>
