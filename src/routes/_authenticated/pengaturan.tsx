...
function SettingsRow({ icon: Icon, title, description, children, onClick }: RowProps) {
  return <button type="button" onClick={onClick} className="flex w-full items-center gap-2.5 rounded-xl p-2.5 text-left transition hover:bg-muted/50 active:bg-muted">
    <span className="grid size-8 shrink-0 place-items-center rounded-lg bg-primary/10 text-primary"><Icon className="size-4" /></span>
    <span className="min-w-0 flex-1"><span className="block text-[13px] font-medium leading-5">{title}</span>{description && <span className="mt-0.5 block text-[10px] leading-4 text-muted-foreground">{description}</span>}</span>
    {children ?? <ChevronRight className="size-3.5 shrink-0 text-muted-foreground" />}
  </button>;
}
...
function SettingsPage() {
  ...
  return <AppShell title="Pengaturan" backTo="/profil" description="Atur pengalaman belajar, akun, dan aplikasi.">
    <div className="space-y-3">
      <Card className="rounded-2xl border-border/70 shadow-sm"><CardHeader className="pb-2"><CardTitle className="text-sm">Belajar</CardTitle></CardHeader><CardContent className="space-y-0.5 p-3">
      ...
      <Card className="rounded-2xl border-border/70 shadow-sm"><CardHeader className="pb-2"><CardTitle className="text-sm">Akun & Keamanan</CardTitle></CardHeader><CardContent className="space-y-0.5 p-3">
      ...
      <Card className="rounded-2xl border-border/70 shadow-sm"><CardHeader className="pb-2"><CardTitle className="text-sm">Bantuan & Informasi</CardTitle></CardHeader><CardContent className="space-y-0.5 p-3">
      ...
      <Card className="rounded-2xl border-border/70 shadow-sm"><CardHeader className="pb-2"><CardTitle className="text-sm">Tentang ENO NIHONGO</CardTitle></CardHeader><CardContent className="space-y-0.5 p-3">
        ...
        <div className="ml-11 flex flex-wrap gap-1.5 pb-2"><button type="button" onClick={() => follow("https://www.instagram.com/enottf/")} className="rounded-lg border px-2.5 py-1 text-[10px] font-medium hover:bg-muted">Instagram · @enottf</button><button type="button" onClick={() => follow("https://www.tiktok.com/@enottff")} className="rounded-lg border px-2.5 py-1 text-[10px] font-medium hover:bg-muted">TikTok · @enottff</button></div>
      </CardContent></Card>

      <Card className="rounded-2xl border-border/70 shadow-sm"><CardHeader className="pb-2"><CardTitle className="text-sm">Status Aplikasi</CardTitle></CardHeader><CardContent className="space-y-2 p-3">
        <div className="flex items-center justify-between"><div><p className="text-xs font-medium">Versi aplikasi</p><p className="text-[10px] text-muted-foreground">ENO NIHONGO</p></div><span className="rounded-full bg-muted px-2.5 py-1 text-[10px] font-semibold">V1.0.0</span></div>
        <div className="flex items-center gap-2 rounded-xl bg-muted/40 p-2.5"><CheckCircle2 className="size-3.5 text-primary" /><div><p className="text-[11px] font-medium">Aplikasi siap digunakan</p><p className="text-[10px] text-muted-foreground">Data dan fitur akan mengikuti pembaruan sistem.</p></div></div>
      </CardContent></Card>
      <div className="pb-4 text-center text-[10px] text-muted-foreground">ENO NIHONGO • Belajar bahasa Jepang dengan terarah.</div>
    </div>
    {open && content[open] && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-2 sm:items-center" role="dialog" aria-modal="true" onClick={() => setOpen(null)}>
      <div className="max-h-[72vh] w-full max-w-md overflow-y-auto rounded-2xl border bg-background p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="mb-3 flex items-center justify-between"><h2 className="text-base font-semibold">{content[open].title}</h2><button type="button" aria-label="Tutup" onClick={() => setOpen(null)} className="rounded-full p-1.5 hover:bg-muted"><X className="size-4" /></button></div>
        <div className="space-y-2.5">{content[open].sections.map((section) => <section key={section.heading} className="rounded-xl bg-muted/40 p-3"><h3 className="mb-1 text-xs font-semibold">{section.heading}</h3><p className="text-xs leading-5 text-muted-foreground">{section.text}</p></section>)}</div>
        <button type="button" onClick={() => setOpen(null)} className="mt-4 w-full rounded-xl bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground">Mengerti</button>
      </div>
    </div>}
  </AppShell>;
}
