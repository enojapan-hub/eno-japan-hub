      ...
      {item && detail && <div className="mt-6 space-y-5">
        ...
        <Card className="mx-auto max-w-2xl overflow-hidden border-border/80 shadow-xl">
          ...
          <CardContent className="space-y-5 p-5 sm:p-7">
            ...
            <section className="rounded-2xl border bg-muted/30 p-5"><h3 className="text-sm font-bold">CONTOH KALIMAT</h3><div className="mt-4 space-y-3">{examples.length ? examples.map((ex, i) => <div key={`${ex.jp ?? "contoh"}-${i}`} className="rounded-xl border bg-background p-4"><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><p lang="ja" className="font-jp text-lg leading-8"><FuriganaText text={ex.jp ?? ""} reading={ex.reading} /></p><p className="mt-2 text-sm leading-6">{ex.id ?? "Arti kalimat sedang diproses…"}</p></div>{ex.jp && <Button type="button" size="icon" variant="ghost" aria-label="Dengarkan contoh kalimat" onClick={() => speak(ex.jp!)}><Volume2 className="size-4" /></Button>}</div></div>) : <p className="text-sm text-muted-foreground">Contoh kalimat belum tersedia.</p>}</div></section>
