...
function QuizRunner() {
  ...
  async function finish() {
    ...
    try {
      await saveAttempt({ quizId: quiz.id, level: quiz.level ?? null, skill: quiz.skill ?? null, total: questions.length, correct: score, durationSeconds: Math.round((Date.now() - startedAt) / 1000), answers: questions.map((q) => ({ questionId: q.id, selectedIndex: answers[q.id] ?? -1, isCorrect: answers[q.id] === q.correct_index })) });
  ...
  if (!current) return <AppShell title={quiz.title}><Card><CardContent className="py-8 text-center text-sm text-destructive">Soal tidak tersedia.</CardContent></Card></AppShell>;

  if (finished) return <AppShell title="Hasil Quiz" description={quiz.title}><Card><CardHeader><CardTitle>Skor kamu</CardTitle></CardHeader><CardContent className="space-y-4"><div className="text-4xl font-bold">{score}/{questions.length}</div><Badge>{Math.round((score / questions.length) * 100)}%</Badge><p className="text-sm text-muted-foreground">{saving ? "Menyimpan hasil…" : "Hasil sudah diproses. XP akan mengikuti aturan akunmu."}</p><div className="flex gap-2"><Button onClick={() => navigate({ to: "/quiz/$slug", params: { slug } })}>Ulangi</Button><Button variant="outline" asChild><Link to="/quiz">Daftar Quiz</Link></Button></div></CardContent></Card></AppShell>;
  ...
  return <AppShell title={quiz.title} description={`${quiz.level} · ${index + 1}/${questions.length}${secondsLeft !== null ? ` · ${Math.floor(secondsLeft / 60)}:${String(secondsLeft % 60).padStart(2, "0")}` : ""}`}>
    <Card><CardHeader><div className="flex items-center justify-between"><Badge>{quiz.skill}</Badge><span className="text-sm text-muted-foreground">Soal {index + 1}</span></div><CardTitle className="pt-3 text-lg">{current.prompt}</CardTitle>{current.prompt_note && <p className="text-sm text-muted-foreground">{current.prompt_note}</p>}</CardHeader><CardContent className="space-y-3">{current.choices.map((choice, i) => <Button key={i} variant={answers[current.id] === i ? "default" : "outline"} className="h-auto w-full justify-start whitespace-normal py-3 text-left" onClick={() => setAnswers((a) => ({ ...a, [current.id]: i }))}>{String.fromCharCode(65 + i)}. {choice}</Button>)}<div className="flex justify-between pt-4"><Button variant="outline" disabled={index === 0} onClick={() => setIndex((i) => i - 1)}>Sebelumnya</Button>{index === questions.length - 1 ? <Button disabled={answers[current.id] === undefined} onClick={finish}>Selesai</Button> : <Button disabled={answers[current.id] === undefined} onClick={() => setIndex((i) => i + 1)}>Berikutnya</Button>}</div></CardContent></Card>
  </AppShell>;
}
