...
import { ArrowRight, BookOpen, CheckCircle2, Flame, Headphones, ListChecks } from "lucide-react";
...
const modules = [
  { to: "/kanji", label: "Kanji", glyph: "漢", tone: "text-emerald-700 bg-emerald-50" },
  { to: "/kotoba", label: "Kotoba", glyph: "言", tone: "text-amber-700 bg-amber-50" },
  { to: "/bunpo", label: "Bunpō", glyph: "文", tone: "text-violet-700 bg-violet-50" },
  { to: "/dokkai", label: "Dokkai", glyph: "読", tone: "text-rose-700 bg-rose-50" },
  { to: "/listening", label: "Listening", glyph: "聴", tone: "text-sky-700 bg-sky-50" },
  { to: "/quiz", label: "Simulasi JLPT", glyph: "JLPT", tone: "text-teal-700 bg-teal-50" },
] as const;
...
function DashboardPage() {
  ...
  return <AppShell compact title="Beranda">
    {isLoading ? <div className="space-y-4"><Skeleton className="h-12 w-3/4 rounded-xl" /><Skeleton className="h-40 w-full rounded-2xl" /><Skeleton className="h-48 w-full rounded-2xl" /><Skeleton className="h-40 w-full rounded-2xl" /></div> : isError ? <Card className="rounded-2xl"><CardContent className="p-5 text-sm">Data belum bisa dimuat. <button className="font-semibold text-primary" onClick={() => refetch()}>Coba lagi</button></CardContent></Card> : <div className="space-y-4">
      ...
      <section><div className="mb-2 flex items-center justify-between px-1"><h2 className="text-[14px] font-semibold">Belajar</h2><Link to="/belajar" className="text-[11px] font-semibold text-primary">Lihat semua</Link></div><div className="grid grid-cols-3 gap-2.5">{modules.map(({ to, label, glyph, tone }) => <Link key={to} to={to as never}><Card className="h-full rounded-xl border-border/60 shadow-sm transition-transform active:scale-[.98] hover:-translate-y-0.5"><CardContent className="flex min-h-[94px] flex-col items-center justify-center gap-2 p-2 text-center"><span className={`grid size-10 place-items-center rounded-xl ${tone}`}><span className="font-jp text-[20px] font-bold leading-none">{glyph}</span></span><span className="text-[11px] font-medium leading-tight">{label}</span></CardContent></Card></Link>)}</div></section>
