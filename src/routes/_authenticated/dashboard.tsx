...
function DashboardPage() {
  ...
  const level = data?.profile?.target_level ?? "N5";
    ...
    <AppShell title={isLoading ? "Beranda" : `Konnichiwa, ${name}`} description="Target harian kamu tersimpan di akun dan berlaku di semua perangkat.">
      {isLoading ? <div className="grid gap-4 sm:grid-cols-3">{[0, 1, 2].map((i) => <Skeleton key={i} className="h-32 w-full rounded-xl" />)}</div> : isError ? <Card className="border-destructive/40"><CardHeader><CardTitle>Gagal memuat data</CardTitle><CardDescription>{(error as Error).message}</CardDescription></CardHeader><CardContent><Button variant="outline" onClick={() => refetch()}>Coba lagi</Button></CardContent></Card> : (
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle>Daily 5</CardTitle><CardDescription>{daily.isLoading ? "Menyiapkan materi hari ini…" : `${daily.data?.completed ?? 0}/${daily.data?.target ?? 5} selesai · ${level}`}</CardDescription></div><Badge variant="secondary">Hari ini</Badge></div></CardHeader>
            <CardContent className="space-y-2">
              {daily.error ? <p className="text-sm text-destructive">Gagal memuat target harian.</p> : daily.data?.items.map((item) => {
                const path = item.type === "kanji" ? "/kanji/$id" : item.type === "vocabulary" ? "/kotoba/$id" : "/bunpo/$id";
              ...
              {!daily.isLoading && !daily.data?.items.length && <p className="text-sm text-muted-foreground">Belum ada materi terbit untuk level ini.</p>}
            </CardContent>
          </Card>
          <Card><CardHeader><CardTitle>Level target: {level}</CardTitle><CardDescription>Semua level N5–N1 terbuka untuk akun gratis. Premium nanti menambah personalisasi, AI, dan analitik.</CardDescription></CardHeader><CardContent><Button asChild variant="outline"><Link to="/profil">Atur target &amp; profil</Link></Button></CardContent></Card>
        </div>
      )}
    </AppShell>
  );
...
function TargetCard({ jp, label, value }: { jp: string; label: string; value: number }) {
  return <Card><CardContent className="flex items-center gap-4 py-6"><span lang="ja" className="font-jp text-3xl leading-none text-primary">{jp}</span><div><p className="text-2xl font-semibold leading-none">{value}</p><p className="mt-1 text-xs text-muted-foreground">{label} / hari</p></div></CardContent></Card>;
}
