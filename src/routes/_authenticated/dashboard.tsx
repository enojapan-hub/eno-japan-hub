...
import { ArrowRight, BookOpen, CheckCircle2, Flame, Headphones, Languages, ListChecks, Newspaper, Trophy, Type, FileText, Sparkles, Target } from "lucide-react";
    ...
    {isLoading ? <div className="space-y-4"><Skeleton className="h-44 w-full rounded-3xl" /><Skeleton className="h-28 w-full rounded-2xl" /></div> : isError ? <Card><CardContent className="p-5 text-sm">Data belum bisa dimuat. <button className="font-semibold text-primary" onClick={() => refetch()}>Coba lagi</button></CardContent></Card> : <div className="relative z-10 space-y-5">
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#24594f] via-[#2f6c60] to-[#45637a] p-5 text-white shadow-lg">
        ...
        <div className="relative">
          ...
          <h1 className="mt-1 max-w-[300px] text-[24px] font-bold leading-tight tracking-tight">Ayo lanjutkan belajar bahasa Jepang.</h1>
          <div className="mt-5 flex items-center gap-3">
            <div className="rounded-2xl bg-white/12 px-3 py-2"><p className="text-[10px] text-white/60">Level</p><p className="mt-0.5 text-sm font-bold">JLPT {level}</p></div>
            <div className="rounded-2xl bg-white/12 px-3 py-2"><p className="text-[10px] text-white/60">Hari ini</p><p className="mt-0.5 text-sm font-bold">{completed}/{target}</p></div>
          </div>
        </div>
      </section>
      ...
      <Card className="rounded-2xl border-border/70 shadow-sm"><CardContent className="p-4">
        <div className="flex items-center justify-between gap-3"><div><p className="text-[11px] font-medium text-muted-foreground">Target belajar hari ini</p><p className="mt-1 text-lg font-bold">{completed} dari {target} selesai</p></div><span className="grid size-10 place-items-center rounded-xl bg-primary/10 text-primary"><Target className="size-5" /></span></div>
        ...
        <div className="mt-2 flex items-center justify-between text-[10px] text-muted-foreground"><span>{Math.round(percent)}% selesai</span><span>{target - completed > 0 ? `${target - completed} lagi` : "Target tercapai 🎉"}</span></div>
      </CardContent></Card>
