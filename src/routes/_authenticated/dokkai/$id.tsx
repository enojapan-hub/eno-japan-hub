...
function renderFurigana(text: string) {
  return text.split(/(\[[^|\]]+\|[^\]]+\])/g).map((part, index) => {
    ...
    return match ? <ruby key={index}>{match[1]}<rt className="text-[0.52em] font-normal tracking-normal">{match[2]}</rt></ruby> : <span key={index}>{part}</span>;
  });
}
...
function DokkaiDetail() {
  ...
  const [fontSize, setFontSize] = useState(0.96);
  ...
  return <AppShell title="読解 — Dokkai" description="Baca dan dengarkan bacaan bahasa Jepang.">
    <div className="mx-auto max-w-3xl pb-8">
      ...
      <Card className="overflow-hidden border-border/80 shadow-none">
        <CardHeader className="border-b border-border/70 px-5 py-4 sm:px-7">
          <div className="flex items-start justify-between gap-3">
            <div><p className="mb-1 text-[11px] font-medium uppercase tracking-[0.14em] text-primary">enonihongo · 読解</p><CardTitle className="text-lg leading-7 sm:text-xl">{p.title}</CardTitle></div>
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            ...
            <Button size="sm" variant="outline" onClick={() => setFontSize((v) => Math.max(0.88, Number((v - 0.06).toFixed(2))))}>A−</Button>
            <Button size="sm" variant="outline" onClick={() => setFontSize((v) => Math.min(1.12, Number((v + 0.06).toFixed(2))))}>A+</Button>
        ...
        <CardContent className="p-0">
          <div className="px-5 py-6 sm:px-8 sm:py-8">
            <div className="mb-5 flex items-center justify-between border-b border-border/70 pb-2.5"><span className="text-xs font-medium text-muted-foreground">Teks bacaan</span><span className="text-[11px] text-muted-foreground">{paragraphs.length} bagian</span></div>
            <article lang="ja" className="font-jp text-foreground" style={{ fontSize: `${fontSize}rem` }}>
              {paragraphs.map((paragraph, index) => {
                ...
                return <div key={index} className="group relative mb-6 last:mb-0">
                  ...
                  <p className="whitespace-pre-wrap text-[1em] leading-[2.05] tracking-[0.01em]">{showFurigana && furiganaParagraphs[index] ? renderFurigana(readingText) : readingText}</p>
                </div>;
              })}
            </article>
            {showTranslation && <div className="mt-7 rounded-lg border border-border/70 bg-muted/40 p-4"><div className="mb-1.5 text-sm font-semibold">Terjemahan</div><p className="whitespace-pre-wrap text-sm leading-6 text-muted-foreground">{p.translation_id || "Terjemahan belum tersedia."}</p></div>}
            {showNotes && <div className="mt-3 rounded-lg border border-primary/20 bg-primary/5 p-4"><div className="mb-1.5 text-sm font-semibold">Catatan belajar</div><p className="text-sm leading-6 text-muted-foreground">Gunakan furigana saat diperlukan dan dengarkan audio untuk membiasakan pelafalan. Cobalah memahami isi bacaan sebelum membuka terjemahan.</p></div>}
          </div>
        </CardContent>
      </Card>

      <div className="mt-5 flex justify-center"><Button variant="outline" size="sm" onClick={() => window.history.back()}><ArrowLeft className="mr-2 h-4 w-4" />Daftar bacaan<ChevronRight className="ml-1 h-4 w-4" /></Button></div>
    </div>
  </AppShell>;
}
