...
import { Loader2, Camera, Globe2, Target, Trophy } from "lucide-react";
...
function ProfilePage() {
  ...
  useEffect(() => { if (!data) return; const name = data.profile?.display_name ?? String(data.user.user_metadata?.["full_name"] ?? data.user.user_metadata?.["name"] ?? ""); const language = data.profile?.ui_language === "en" || data.profile?.ui_language === "ja" ? data.profile.ui_language : "id"; setForm({ display_name: name, target_level: (LEVELS as readonly string[]).includes(data.profile?.target_level ?? "") ? (data.profile?.target_level as Level) : "N5", ui_language: language, daily_kanji_target: data.settings?.daily_kanji_target ?? 5, daily_vocab_target: data.settings?.daily_vocab_target ?? 10, daily_grammar_target: data.settings?.daily_grammar_target ?? 5, furigana_enabled: data.settings?.furigana_enabled ?? true, daily_reminder: data.settings?.daily_reminder ?? false }); }, [data]);
  ...
  if (isLoading || !form) return <AppShell title="Profil" description="Kelola akun dan target belajar kamu."><div className="space-y-4"><Skeleton className="h-44 w-full rounded-2xl" /><Skeleton className="h-64 w-full rounded-2xl" /></div></AppShell>;
  ...
  return <AppShell title="Profil" description="Kelola akun dan target belajar kamu."><form onSubmit={submit} className="space-y-4" noValidate>
    <Card className="overflow-hidden rounded-2xl border-border/70 shadow-sm"><CardContent className="p-5"><div className="flex items-center gap-4"><div className="relative shrink-0"><div className="grid size-20 place-items-center overflow-hidden rounded-full border-4 border-background bg-primary/10 text-2xl font-bold text-primary shadow-sm">{avatar ? <img src={avatar} alt="Avatar pengguna" className="size-full object-cover" /> : name.slice(0, 1).toUpperCase()}</div><span className="absolute -bottom-1 -right-1 grid size-7 place-items-center rounded-full border-2 border-background bg-primary text-primary-foreground"><Camera className="size-3.5" /></span></div><div className="min-w-0 flex-1"><p className="truncate text-base font-semibold">{name}</p><p className="mt-0.5 truncate text-xs text-muted-foreground">{data.user.email ?? "Akun Google"}</p><div className="mt-3 flex flex-wrap gap-2"><span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-semibold text-emerald-700"><Target className="size-3.5" /> {form.target_level}</span><span className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-2.5 py-1 text-[11px] font-semibold text-amber-700"><Trophy className="size-3.5" /> {points.toLocaleString("id-ID")} poin</span></div></div></div></CardContent></Card>
    ...
    {formError && <p role="alert" className="text-sm text-destructive">{formError}</p>}<Button type="submit" className="w-full rounded-xl" disabled={mutation.isPending}>{mutation.isPending && <Loader2 aria-hidden className="size-4 animate-spin" />}Simpan perubahan</Button>
  </form></AppShell>;
}
