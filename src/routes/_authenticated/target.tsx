...
function TargetPage() {
  ...
  const allDone = target > 0 && completed >= target;

  return (
    <AppShell compact title="Target">
      <div className="mx-auto max-w-3xl space-y-4 eno-rise">
        <section>
          <div className="mb-3 flex items-start justify-between gap-3">
            ...
            <span className={allDone?"grid size-10 shrink-0 place-items-center rounded-2xl bg-primary text-primary-foreground eno-bounce":"grid size-10 shrink-0 place-items-center rounded-2xl bg-primary/10 text-primary eno-pulse-soft"}><Target className="size-5" /></span>
          </div>

          <Card className={allDone?"rounded-2xl border-primary/35 shadow-sm eno-pop":"rounded-2xl border-border/70 shadow-sm eno-rise"}>
            <CardContent className="p-4">
              <div className="grid grid-cols-4 gap-2 text-center">
                <div className="rounded-xl bg-muted/45 p-2"><Flame className="mx-auto size-4 text-orange-500 eno-pulse-soft" /><p className="mt-1 text-[12px] font-bold">—</p><p className="text-[9px] text-muted-foreground">Streak</p></div>
              ...
              <p className={allDone?"mt-2 text-[10px] font-semibold text-primary eno-pop":"mt-2 text-[10px] text-muted-foreground"}>{target ? allDone ? "Target hari ini selesai! 🎉" : `${completed} dari ${target} aktivitas selesai` : "Planner sedang menyiapkan targetmu."}</p>
            </CardContent>
          </Card>
        </section>

        <section>
          <div className="mb-2 flex items-center justify-between px-1"><h2 className="text-[13px] font-semibold">Rencana Belajar Hari Ini</h2><Sparkles className="size-4 text-primary eno-pulse-soft" /></div>
          <div className="space-y-2">
            {adaptive.data?.tasks?.length ? adaptive.data.tasks.map((task) => {
              ...
              const isDone = done >= task.target_count && task.target_count > 0;
              const to = taskLinks[task.task_type] || "/belajar";
              return (
                <Link key={task.id} to={to as "/belajar"} className={isDone?"block rounded-2xl border border-primary/25 bg-primary/[.035] p-3 transition hover:border-primary/35 eno-pop":"block rounded-2xl border bg-card p-3 transition hover:border-primary/35 eno-rise"}>
                  <div className="flex items-start gap-3">
                    <span className={isDone ? "grid size-8 shrink-0 place-items-center rounded-xl bg-primary text-primary-foreground eno-bounce" : "grid size-8 shrink-0 place-items-center rounded-xl bg-muted text-muted-foreground"}><CheckCircle2 className="size-4" /></span>
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-primary eno-progress" style={{ width: `${taskPercent}%` }} /></div>
                      <p className={isDone?"mt-1 text-[9px] font-semibold text-primary":"mt-1 text-[9px] text-muted-foreground"}>{isDone ? "Selesai ✓" : done > 0 ? "Sedang dikerjakan" : "Belum dimulai"}</p>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {quizzes.map(({ label, icon: Icon, to }) => <Link key={label} to={to} className="rounded-2xl border bg-card p-3 transition hover:border-primary/35 eno-rise"><Icon className="size-4 text-primary" /><p className="mt-2 text-[11px] font-semibold">{label}</p></Link>)}
          </div>
        </section>

        <Card className="rounded-2xl border-primary/20 bg-primary/[0.04] eno-rise">
          <CardContent className="flex items-center gap-3 p-4">
            <span className="grid size-10 place-items-center rounded-2xl bg-primary/10 text-primary eno-pulse-soft"><Brain className="size-5" /></span>
        ...
        <section>
          ...
          <Card className="rounded-2xl eno-rise"><CardContent className="divide-y p-1">{missions.map((mission) => <div key={mission.label} className="flex items-center gap-3 px-3 py-2.5"><span className="grid size-7 place-items-center rounded-lg bg-muted"><CheckCircle2 className="size-3.5 text-muted-foreground" /></span><p className="min-w-0 flex-1 text-[11px] font-medium">{mission.label}</p><div className="text-right"><p className="text-[10px] font-bold text-primary">+{mission.xp} XP</p><p className="text-[9px] text-amber-600">+{mission.points} Pts</p></div></div>)}</CardContent></Card>
        </section>
      </div>
    </AppShell>
  );
