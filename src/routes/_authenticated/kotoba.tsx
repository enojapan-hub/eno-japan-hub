...
function KotobaPage(){
  ...
  const {data:masteredRows}=useQuery({queryKey:["mastered-items","vocabulary",level],enabled:!!targetLevel,queryFn:async()=>{const {data:auth}=await supabase.auth.getUser();if(!auth.user)return [] as Array<{item_id:string}>;const {data,error}=await supabase.from("user_item_progress").select("item_id").eq("user_id",auth.user.id).eq("item_type","vocabulary").eq("level",level).eq("status","mastered");if(error)throw error;return (data??[]) as Array<{item_id:string}>;}});
  ...
  const mutation=useMutation({mutationFn:(id:string)=>markItemLearned({itemType:"vocabulary",itemId:id,level}),onSuccess:(_,id)=>{setLearned(v=>({...v,[id]:true}));void qc.invalidateQueries({queryKey:["my-progress"]});void qc.invalidateQueries({queryKey:["mastered-items","vocabulary",level]});void qc.invalidateQueries({queryKey:["leaderboard"]})}});
  ...
  return <AppShell title="Kotoba" backTo="/belajar" backLabel="Materi" compact>{levelLoading?<p className="py-8 text-center text-xs text-muted-foreground">Memuat level…</p>:levelError?<p className="py-8 text-center text-xs text-destructive">Level profil tidak dapat dimuat.</p>:<div className="mx-auto max-w-md">
    {index==null?<>
      ...
      <div className="mb-3 flex items-center justify-between"><button onClick={()=>setIndex(null)} className="flex items-center gap-1.5 text-[12px] font-semibold"><ArrowLeft className="size-4"/>Kosakata {level}</button><span className="text-[11px] font-semibold text-muted-foreground">{index+1} / {cards.length}</span></div>
      <div className="rounded-2xl border bg-card p-4 shadow-sm">
        <div className="flex items-center justify-between"><button onClick={prev} disabled={index===0} className="grid size-10 place-items-center rounded-full bg-primary/8 text-primary disabled:opacity-30"><ArrowLeft className="size-5"/></button><div className="max-w-[240px] text-center"><div lang="ja" className="font-jp text-[42px] font-bold leading-tight">{item.term}</div>{item.reading&&<p className="mt-1 font-jp text-[13px] text-muted-foreground">{item.reading}</p>}<p className="mt-1 text-[15px] font-semibold">{item.meaning_id||item.source_meaning_id||"Arti belum tersedia"}</p></div><button onClick={next} disabled={index===cards.length-1} className="grid size-10 place-items-center rounded-full bg-primary/8 text-primary disabled:opacity-30"><ArrowRight className="size-5"/></button></div>
        <div className="mt-3 flex items-center justify-center gap-2"><button onClick={()=>speak(item.term)} className="grid size-11 place-items-center rounded-full bg-primary text-primary-foreground"><Volume2 className="size-5"/></button><button onClick={()=>setReview(v=>({...v,[item.id]:!v[item.id]}))} className="grid size-10 place-items-center rounded-full border"><Star className={`size-4 ${review[item.id]?"fill-current text-primary":""}`}/></button></div>
        <div className="mt-4 grid grid-cols-2 overflow-hidden rounded-xl border"><div className="bg-primary/6 p-2.5"><p className="text-[9px] text-muted-foreground">Kelas Kata</p><p className="mt-1 text-[11px] font-semibold">{item.part_of_speech||senses[0]?.part_of_speech||"—"}</p></div><div className="border-l bg-primary/6 p-2.5"><p className="text-[9px] text-muted-foreground">Arti</p><p className="mt-1 text-[11px] font-semibold">{item.meaning_id||item.source_meaning_id||"—"}</p></div></div>
        {(senses[0]?.usage_note_id||senses.length>1)&&<section className="mt-3 rounded-xl bg-muted/35 p-3"><h2 className="text-[10px] font-bold">Penggunaan</h2>{senses.slice(0,3).map((s,i)=><p key={i} className="mt-1 text-[11px] leading-5 text-muted-foreground">{s.usage_note_id||s.meaning_id}</p>)}</section>}
        {examples.length>0&&<section className="mt-4"><h2 className="text-[12px] font-bold">Contoh Kalimat</h2>{examples.slice(0,3).map((e,i)=><div key={i} className="mt-2 text-[11px] leading-5"><p lang="ja" className="font-jp text-[13px]">{e.jp}</p>{e.reading&&<p className="text-muted-foreground">{e.reading}</p>}{e.id&&<p className="text-muted-foreground">{e.id}</p>}</div>)}</section>}
        <div className="mt-4 grid grid-cols-2 gap-2"><Button variant="outline" className="h-10 rounded-full text-[11px]" onClick={()=>setReview(v=>({...v,[item.id]:true}))}><Star className="mr-1.5 size-3.5"/>Tambah ke Review</Button><Button className="h-10 rounded-full text-[11px]" onClick={()=>mutation.mutate(item.id)} disabled={learned[item.id]||mutation.isPending}>{learned[item.id]?<><Check className="mr-1.5 size-3.5"/>Sudah Hafal</>:"Sudah Hafal · +5 XP +5 Poin"}</Button></div>
