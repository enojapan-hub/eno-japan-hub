...
function KotobaPage(){
  ...
  return <AppShell title="Kotoba" backTo="/belajar" backLabel="Materi" compact>{levelLoading?<p className="py-8 text-center text-xs text-muted-foreground">Memuat level…</p>:levelError?<p className="py-8 text-center text-xs text-destructive">Level profil tidak dapat dimuat.</p>:<div className="mx-auto max-w-md">
    {index==null?<><div className="mb-3 flex items-end justify-between"><h1 className="text-[20px] font-bold tracking-tight">Kosakata {level}</h1><span className="text-[11px] font-semibold text-muted-foreground">{allCards.length}</span></div>
      ...
      item?<div onTouchStart={e=>touch.current=e.touches[0].clientX} onTouchEnd={e=>finishSwipe(e.changedTouches[0].clientX)} className="pb-2">
        ...
        <section className="px-1">
          <div className="grid grid-cols-[42px_1fr_42px] items-center gap-2">
            <button onClick={prev} disabled={index===0} className="grid size-10 place-items-center rounded-full bg-primary/[0.07] text-primary disabled:opacity-25"><ArrowLeft className="size-5"/></button>
            <div className="min-w-0 text-center"><div lang="ja" className="font-jp text-[48px] font-bold leading-none tracking-tight">{item.term}</div>{item.reading&&<p lang="ja" className="mt-2 font-jp text-[15px] text-muted-foreground">{item.reading}</p>}<p className="mt-1 text-[16px] font-medium">{item.meaning_id||item.source_meaning_id||"Arti belum tersedia"}</p></div>
            <button onClick={next} disabled={index===allCards.length-1} className="grid size-10 place-items-center rounded-full bg-primary/[0.07] text-primary disabled:opacity-25"><ArrowRight className="size-5"/></button>
          </div>
