...
function KotobaPage() {
  ...
  const { data, isLoading, error } = useQuery({ queryKey: ["vocab", level], queryFn: () => fetchVocabList(level), enabled: !!targetLevel });
  ...
  const usageNotes = item ? [
    part.toLowerCase().includes("verb") || part.toLowerCase().includes("kata kerja") ? "Percakapan sehari-hari untuk menyatakan tindakan atau aktivitas." :
  ...
  return <AppShell title="言葉 · Kotoba" description={`Kosakata ${level} dengan format belajar lengkap: kanji, bacaan, romaji, arti Indonesia, contoh kalimat, penjelasan, konteks, audio, dan latihan.`} backTo="/belajar" backLabel="Belajar">
    {levelLoading ? <p className="mt-8 text-center">Memuat level belajar…</p> : levelError ? <p className="mt-8 text-center text-destructive">Level profil tidak dapat dimuat.</p> : <>
      ...
      {item && <div className="mt-6"><StudyFlashcard index={index} total={cards.length} level={item.level} title={item.term} reading={item.reading} romaji={item.romaji} meaning={meaning} secondary={part} explanation={explanation} usageNotes={usageNotes} examples={examples} question={choices.length > 1 ? { prompt: `Apa arti kosakata ${item.term}?`, choices, correctIndex: 0 } : null} learned={!!learned[item.id]} kotobaMode onLearned={() => mutation.mutate(item.id)} onPrev={() => setIndex(i => Math.max(0, i - 1))} onNext={() => setIndex(i => Math.min(cards.length - 1, i + 1))} /></div>}
      {!isLoading && !cards.length && <p className="mt-8 text-center text-sm text-muted-foreground">Belum ada kosakata untuk {level}.</p>}
    </>}
  </AppShell>;
}
