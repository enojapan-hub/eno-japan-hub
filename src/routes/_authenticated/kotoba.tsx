...
async function fetchKotobaRows(level:Level):Promise<VocabRow[]>{
  const extended=await supabase.from("vocabulary").select("id,term,reading,romaji,meaning_id,part_of_speech,examples,level,source_book,lesson_number,lesson_title,sort_order").eq("level",level).eq("is_published",true).order("lesson_number",{ascending:true,nullsFirst:false}).order("sort_order",{ascending:true});
  if(!extended.error)return (extended.data??[]) as VocabRow[];
  const basic=await supabase.from("vocabulary").select("id,term,reading,meaning_id,part_of_speech,examples,level,sort_order").eq("level",level).eq("is_published",true).order("sort_order",{ascending:true});
  if(basic.error)throw new Error(basic.error.message);
  return (basic.data??[]) as VocabRow[];
}
...
function KotobaPage(){
  ...
  const {data,isLoading,error}=useQuery({queryKey:["vocab-primary",level],queryFn:()=>fetchKotobaRows(level),enabled:!!targetLevel,retry:1});
