    ...
    <AppShell title="Kotoba" description="Pelajari kosakata Jepang dengan arti, cara baca, contoh kalimat, dan penjelasan penggunaan." backTo="/belajar" backLabel="Belajar">
      <div className="mx-auto max-w-3xl space-y-4">
        ...
        <Card className="overflow-hidden rounded-3xl border-border/70 shadow-sm"><CardContent className="p-0">
          ...
          <div className="grid grid-cols-2 gap-2 border-t p-5 sm:grid-cols-3"><Button variant="outline" onClick={() => setIndex(i => Math.max(0, i - 1))} disabled={index === 0}><ArrowLeft className="mr-2 size-4" />Sebelumnya</Button><Button variant={learned[item.id] ? "secondary" : "outline"} onClick={() => item.id.startsWith("fallback-") ? undefined : mutation.mutate(item.id)} disabled={!!learned[item.id] || item.id.startsWith("fallback-")} className="col-span-2 sm:col-span-1">{learned[item.id] ? <><Check className="mr-2 size-4" />Sudah dipelajari</> : "Tandai dipelajari"}</Button><Button onClick={() => setIndex(i => Math.min(cards.length - 1, i + 1))} disabled={index === cards.length - 1} className="col-span-2 sm:col-span-1">Berikutnya<ArrowRight className="ml-2 size-4" /></Button></div>
