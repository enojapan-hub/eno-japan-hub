...
function DokkaiPage() {
  ...
  return <AppShell title="読解 · Dokkai" description="Baca teks Jepang, dengarkan pelafalan, lalu cek pemahaman." backTo="/belajar" backLabel="Belajar">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      {data?.map((p) => <Card key={p.id} className="group overflow-hidden shadow-none transition-all hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md">
        <CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div><p className="mb-1 flex items-center gap-1.5 text-xs font-medium text-primary"><BookOpen className="size-3.5" /> Reading practice</p><CardTitle className="text-lg leading-7">{p.title}</CardTitle><p className="mt-1 text-xs text-muted-foreground">Bacaan tingkat {p.level}</p></div><Badge variant="secondary">{p.level}</Badge></div></CardHeader>
