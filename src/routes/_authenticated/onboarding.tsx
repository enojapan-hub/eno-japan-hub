...
import { Camera, Check, ChevronLeft, ChevronRight, Globe2, GraduationCap, Loader2, UserRound } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/onboarding")({ head: () => ({ meta: [{ title: "Mulai di enonihongo" }] }), component: OnboardingPage });

const LEVELS = ["N5", "N4", "N3", "N2", "N1"] as const;
type Level = (typeof LEVELS)[number];
type Language = "id" | "en" | "ja";
const LANGUAGES: { value: Language; label: string; sub: string }[] = [
  { value: "id", label: "Bahasa Indonesia", sub: "Indonesia" },
...
async function loadProfile() {
  ...
  const { data: profile, error } = await supabase.from("profiles").select("display_name,target_level,ui_language").eq("id", auth.user.id).maybeSingle();
...
function OnboardingPage() {
  ...
  useEffect(() => {
    loadProfile().then(({ user, profile }) => {
      const meta = user.user_metadata ?? {};
      if (meta["onboarding_completed"] === true) { navigate({ to: "/dashboard", replace: true }); return; }
      ...
      setLanguage(profile?.ui_language === "en" || profile?.ui_language === "ja" ? profile.ui_language : "id");
      setAvatar(String(meta["avatar_url"] ?? meta["picture"] ?? ""));
    }).catch(e => setError(e instanceof Error ? e.message : "Gagal memuat profil.")).finally(() => setLoading(false));
  }, [navigate]);

  async function save() {
    if (name.trim().length < 2) { setError("Nama tampilan minimal 2 karakter."); setStep(0); return; }
    setError(null); setSaving(true);
    try {
      const { data: auth, error: authError } = await supabase.auth.getUser();
      if (authError || !auth.user) throw new Error("Sesi masuk tidak ditemukan.");
      const id = auth.user.id;
      const { error: profileError } = await supabase.from("profiles").update({ display_name: name.trim(), target_level: level, ui_language: language }).eq("id", id);
      if (profileError) throw new Error(`Profil gagal disimpan: ${profileError.message}`);
      await supabase.auth.updateUser({ data: { display_name: name.trim(), avatar_url: avatar || null, onboarding_completed: true } });
      toast.success("Profil berhasil disiapkan. Selamat belajar di enonihongo!");
      navigate({ to: "/dashboard", replace: true });
    } catch (e) { const message = e instanceof Error ? e.message : "Profil gagal disimpan."; setError(message); toast.error(message); } finally { setSaving(false); }
  }
  ...
  const steps = [
    ...
    { title: "Pilih foto profil", desc: "Biar profil enonihongo terasa lebih personal.", icon: Camera },
    { title: "Pilih level Jepang", desc: "Materi dan target belajar akan disesuaikan.", icon: GraduationCap },
    { title: "Bahasa yang digunakan", desc: "Pilih bahasa tampilan aplikasi.", icon: Globe2 },
  ];
  ...
  return <div className="min-h-screen bg-background px-4 py-8 sm:py-12"><div className="mx-auto w-full max-w-lg">
    ...
    <Card className="rounded-3xl border-border/70 shadow-sm"><CardContent className="p-5 sm:p-7">
      ...
      {step === 3 && <div className="grid gap-3">{LANGUAGES.map(l => <button type="button" key={l.value} onClick={() => setLanguage(l.value)} className={`flex items-center justify-between rounded-2xl border p-4 text-left transition ${language === l.value ? "border-primary bg-primary/5 ring-2 ring-primary/15" : "hover:bg-muted/50"}`}><span><span className="block text-base font-semibold">{l.label}</span><span className="text-xs text-muted-foreground">{l.sub}</span></span>{language === l.value && <Check className="size-5 text-primary" />}</button>)}</div>}
      {error && <p role="alert" className="mt-4 text-sm text-destructive">{error}</p>}
    </CardContent></Card>
    <div className="mt-5 flex gap-3"><Button type="button" variant="outline" className="h-11 flex-1 rounded-xl" disabled={saving} onClick={() => step === 0 ? navigate({ to: "/dashboard", replace: true }) : setStep(step - 1)}><ChevronLeft className="size-4" />{step === 0 ? "Nanti" : "Kembali"}</Button>{step < steps.length - 1 ? <Button type="button" className="h-11 flex-1 rounded-xl" onClick={() => { if (step === 0 && name.trim().length < 2) { setError("Nama tampilan minimal 2 karakter."); return; } setError(null); setStep(step + 1); }}><span>Lanjut</span><ChevronRight className="size-4" /></Button> : <Button type="button" className="h-11 flex-1 rounded-xl" disabled={saving} onClick={() => void save()}>{saving ? <Loader2 className="size-4 animate-spin" /> : <Check className="size-4" />}Selesai</Button>}</div>
    <p className="mt-5 text-center text-[11px] text-muted-foreground">Kamu bisa mengubah semua pilihan ini lagi dari Profil.</p>
  </div></div>;
}
